...
2. **거리 3.0 이상**: 플레이어에게 접근 (이동속도 0.8)
3. **거리 3.0~5.0 (최적 거리)**: 정지 후 공격
   - 머리 위에 '!' 경고 표시 (0.5초)
   - 투사체 발사 (속도 3.0, 데미지 2)
4. **거리 3.0 미만**: 플레이어로부터 후퇴 (이동속도 0.8)

**구현 필요 요소**:
- 투사체 프리팹 (Bullet 클래스 활용 가능)
- 경고 UI 스프라이트 ('!')
- 후퇴 AI 로직
...
**특징**:
- 높은 체력과 방어력을 가진 탱커형 적
- **전투 역할**: 다른 적들의 "벽", 플레이어 화력 분산
- **핵심 메커니즘**: 방어력 50%로 처치 시간 2배 소요
...
4. **패시브**: 받는 데미지 50% 감소 (방어력)

**구현 필요 요소**:
- 방어력 시스템 (CharacterBase에 추가 필요)
- 데미지 감소 계산 로직
...
2. 1.0 거리 이내 진입 시 자폭 준비
   - 0.5초간 빠르게 점멸 (시각 경고)
   - 이동 정지
3. 폭발 발동
   - 범위(2.0) 내 모든 대상에게 데미지 5
   - 폭발 이펙트
4. 자신 사망 (드랍 없음)
